view: bauer_list {
  ...
  dimension: age_as_number {
    hidden: yes
    sql: CAST(CASE WHEN ${TABLE}.age LIKE '%-%' THEN '0'
  ...
  dimension: gender {
    ...
    case: {
      when: {
        sql: ${TABLE}.gender = 'f' or ${TABLE}.gender = 'Female' ;;
        label: "F"
      }

      when: {
        sql: ${TABLE}.gender = 'm' or ${TABLE}.gender = 'Male' ;;
        label: "M"
      }
  ...
  dimension: home_mobile_number {
    hidden: no
    type: string
    sql: COALESCE (${TABLE}.mobile_number,'')  || '-' || COALESCE (${TABLE}.telephonehomenumber, '')
      ;;
  }
