view: ga_sessions_planetradio_advcompsearch {
  ...
  derived_table: {
    sql: SELECT
      ...
      FROM
        ( SELECT * FROM {% table_date_range ga_sessions_planetradio.date_filter 127467161.ga_sessions_ %},
      {% table_date_range ga_sessions_planetradio.date_filter 127467161.ga_sessions_intraday_ %})
      GROUP BY 1
      ORDER BY 1
       ;;
  }
  ...
  dimension: Brands_Visited {
    label: "Pages Visited"
    description: "In order to cars by users, filter this field and do \"contains\" and a page path, for example \"beauty/\"."
    type: string
