- view: jr_lifestyle_referral_source_medium
  ...
  sql: |
      ...
      WHERE {% condition brand_filter %} RegEXP_EXTRACT(hits.page.pagePath, r'^\/(.+?)\/.+') {% endcondition %} AND {% condition referral_Medium %} trafficSource.medium {% endcondition %} AND hits.type = 'PAGE'
