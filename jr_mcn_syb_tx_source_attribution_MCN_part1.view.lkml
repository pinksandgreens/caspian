view: jr_mcn_syb_tx_source_attribution_MCN_part1 {
  label: "MCN"
  ...
  derived_table: {
    sql:
      SELECT
        ...
        REGEXP_EXTRACT(Core_Stats.pagePath, r'^(.+\/)default.aspx') AS PageURL,
        ...
        IF(Core_Stats.hits.hitNumber = 1,REGEXP_EXTRACT(Core_Stats.pagePath, r'^(.+\/)default.aspx'),NULL) AS LandingPage,
      ...
      FROM
        ...
        (SELECT
          ...
          IF(REGEXP_MATCH(hits.page.pagePath, r'^.+?\/(.+?)\/.*$'),REGEXP_EXTRACT(hits.page.pagePath, r'^.+?\/(.+?)\/.*$'),'MCN') AS Category,
          FORMAT_UTC_USEC(visitStartTime*1000000) AS SessionStartDateTime,
          hits.hitNumber,
        ...
        FROM
          (TABLE_DATE_RANGE([uplifted-light-89310:22661559.ga_sessions_], DATE_ADD(USEC_TO_TIMESTAMP(now()), -62, 'DAY'), USEC_TO_TIMESTAMP(now())))
