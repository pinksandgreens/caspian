...
;;
  ...
  dimension: 30_day_buckets  {
    ...
    sql:  CASE
                WHEN DATEDIFF(DATE_ADD(CURRENT_TIMESTAMP(),-3,"DAY"),TIMESTAMP(${TABLE}.Date)) BETWEEN 0 AND 29 THEN 1 --Amended Range for Gregs Presentation
                WHEN DATEDIFF(DATE_ADD(CURRENT_TIMESTAMP(),-3,"DAY"),TIMESTAMP(${TABLE}.Date)) BETWEEN 30 AND 59 THEN 2 --Amended Range for Gregs Presentation
            END ;;
  }
